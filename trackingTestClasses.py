...
# Some autograder tests rely on specific ordering of dictionary items
# when choosing ghost actions. In Python 2 ordering of dictionary
# items was arbitrary but constant, in Python 3 it is randomized. This
# maps items to the same ordering as in Python 2.
def cmpGhostActions(direction, allPossibleActions):
    if 'West' not in allPossibleActions:
        python2_ordering = ['North', 'East', 'Stop', 'South']
    else:
        python2_ordering = ['West', 'East', 'Stop', 'South', 'North']
    return python2_ordering.index(direction)
