...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        self.beliefs is a Counter object (similar to a dictionary, from
        util.py) that stores for every position Pacman's current beliefs
        about how likely it is that the ghost is at that position. This
        should get updated in this method.
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                allPossible[p] = self.beliefs[p] * emissionModel[true_distance]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old_pos, old_prob in self.beliefs.items():
            if old_prob > 0:
                new_pos_dist = self.getPositionDistribution(self.setGhostPosition(gameState, old_pos))
                for new_pos, new_prob in new_pos_dist.items():
                    new_beliefs[new_pos] += new_prob * old_prob
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        Each particle is a tuple of ghost positions. Note that ghosts
        may occupy the same space. Use self.legalPositions to obtain a
        list of positions a ghost may occupy. And use
        ...
        turn an iterator object into a list using the built-in
        function `list`. Also keep in mind that tuples in the
        cartesian product are not returned in a random order; you must
        shuffle the result in order to ensure even placement of
        particles across the board.

        Note: as before the variable you store your particles in must
        be a list; a list is simply a collection of unweighted
        variables (tuples of positions in this case).  Storing your
        particles as a Counter (where there could be an associated
        weight with each position) is incorrect and may produce
        errors.

        """
        ghosts = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(ghosts)
        ghost_len = len(ghosts)
        self.particles = [ghosts[i % ghost_len] for i in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        """
        pacman_position = gameState.getPacmanPosition()
        noisy_distances = gameState.getNoisyGhostDistances()
        if len(noisy_distances) < self.numGhosts:
            return
        emission_models = [busters.getObservationDistribution(dist) for dist in noisy_distances]

        self.update_jailed_ghosts(gameState)

        weights = util.Counter()
        for particle in self.particles:
            weight = 1
            for i in range(self.numGhosts):
                if noisy_distances[i] is not None:
                    true_dist = util.manhattanDistance(pacman_position, particle[i])
                    weight *= emission_models[i][true_dist]
    ...
    def update_jailed_ghosts(self, game_state):
        noisy_distances = game_state.getNoisyGhostDistances()
        for j in range(self.numGhosts):
            if noisy_distances[j] is None:
                for i, particle in enumerate(self.particles):
                    self.particles[i] = self.getParticleWithGhostInJail(particle, j)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                new_pos_dist = getPositionDistributionForGhost(
                    setGhostPositions(gameState, new_particle), i, self.ghostAgents[i])
