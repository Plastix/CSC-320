...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hits_wall:
                next_cell = (nextx, nexty)
                if next_cell in self.corners:
                    corners = update_corner(corners, self.corners.index(next_cell))
                new_state = (next_cell, corners)
...
def update_corner(tup, index):
    return tuple(map(lambda i: True if i == index else tup[i], range(len(tup))))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        return search.astar(problem,
                            lambda position, prob: min(map(
                                lambda cell: util.manhattanDistance(cell, position),
                                food.asList()
                            )))
